...
| 2026-07-01 | `src/pipelines/train_pipeline.py`, `config/classification.yaml` | Reverted `train_classification` to the Feb 2026 baseline: plain Adam (no scheduler, no cosine annealing), hardcoded patience=10 early stopping, simple epoch loop. Only addition is the conv1 1-channel adaptation (necessary since data pipeline now outputs 1-channel). Removed scheduler/early_stopping blocks from config. |
